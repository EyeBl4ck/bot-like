...
async def add_saldo(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    "💠 Realizar Depósito PIX", callback_data="add_saldo_auto"
                ),
    ...
    await m.edit_message_text(
        """<b>💵 Adicionar Saldo</b>
<i>Aqui abaixo vc poderá adicionar saldo para realizar suas compras!</i>""",
        reply_markup=kb,
    )
...
<b>⚠️ Se vc já fez o pagamento, envie o comprovante para @VigaristaTrampos
{details}""",
        reply_markup=kb,
    )
...
- Gift card: <code>{gift}</code>"""
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores GG's",url="https://t.me/Savitarstoresbot"
                            ),
                        ],
                    ]
                )
...
- Gift card: <code>{gift[0:6]}</code>\n<a href='https://t.me/savitarstoresbot'>Recarregar</a>"""
    ...
    if refer:
        ...
        if quantity > 0:
            ...
            await c.send_message(
                int(refer),
                f"💸 Seu convidado {mention} adicionou saldo na Lojinha e você recebeu {quantity} de saldo.",
            )
